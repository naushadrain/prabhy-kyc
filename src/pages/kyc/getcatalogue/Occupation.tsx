...
export function Occupation({ value, onChange, label = "Occupation" }: HonourProps) {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                setLoading(true);
                setError("");
                const list = await fetchCatalogueList(OCCUPATION_TYPE, 1);
                if (alive) setItems(list);
            } catch (e: any) {
                if (alive) setError(e?.message || "Failed to load occupation list");
            } finally {
        ...
        <div>
            ...
            <Select value={selected} onValueChange={handleChange}>
                <SelectTrigger className="mt-2">
                    <SelectValue placeholder={loading ? "Loading..." : "Select occupation"} />
                </SelectTrigger>
